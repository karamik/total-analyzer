...
# TOTAL Protocol Analyzer — v2.0

**SAST + Hardware Security Advisor**  
*By International Group of Developers*

TOTAL Analyzer is a static analysis tool that:
- Finds **SQL injections** (cross‑file taint tracking) in Python code (Flask, FastAPI, Django ORM)
...
```bash
# Pull the image
docker pull ghcr.io/karamik/total-analyzer:latest

# Run on your project
cd your-python-project
docker run --rm -v $(pwd):/src ghcr.io/karamik/total-analyzer:latest /src --sarif > results.sarif
...
- **Cross‑function calls** (inter‑procedural taint tracking)
...
```bash
docker run --rm -v /path/to/your/code:/src ghcr.io/karamik/total-analyzer:latest /src --sarif
```
...
See [LIMITATIONS.md](./LIMITATIONS.md) – no support for dynamic imports, Python only.
