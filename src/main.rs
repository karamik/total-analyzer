...
// SARIF structures (simplified but GitHub-compatible)
#[derive(Serialize)]
...
impl<'a> TotalChecker<'a> {
    ...
    fn is_expr_tainted(&self, expr: &Expr) -> bool {
        match expr {
            ...
            Expr::FString(fstring) => fstring
                .values
                .iter()
                .any(|part| matches!(part, ast::FStringPart::Value(val) if self.is_expr_tainted(&val.expression))),
            Expr::BinOp(binop) => {
                // Поддерживаем сложение (+) и форматирование (%)
                match binop.op {
                    ast::Operator::Add => self.is_expr_tainted(&binop.left) || self.is_expr_tainted(&binop.right),
                    ast::Operator::Mod => self.is_expr_tainted(&binop.left) || self.is_expr_tainted(&binop.right),
                    _ => false,
                }
            }
            Expr::Call(call) => {
                // If it's a sanitizer, it's clean. Otherwise, check arguments.
    ...
    fn is_sanitizer(&self, call: &ast::ExprCall) -> bool {
        if let Expr::Name(name) = call.func.as_ref() {
            return matches!(name.id.as_str(), "int" | "float" | "bindparam");
        }
        false
    }
    ...
    fn is_db_sink(&self, call: &ast::ExprCall) -> bool {
        ...
        match call.func.as_ref() {
            Expr::Attribute(attr) => {
                // Поддерживаем .execute(), .raw(), .executemany()
                if dangerous.contains(&attr.attr.as_str()) {
                    return true;
                }
                // Django ORM: Model.objects.raw()
                if attr.attr.as_str() == "raw" {
                    return true;
                }
                false
            }
            Expr::Name(name) => dangerous.contains(&name.id.as_str()),
            _ => false,
        }
    }
...
impl<'a> Visitor<'a> for TotalChecker<'a> {
    fn visit_stmt(&mut self, stmt: &'a Stmt) {
        // 1. Detect taint sources: function parameters with web decorators
        if let Stmt::FunctionDef(def) = stmt {
            if def.decorator_list.iter().any(|d| self.is_web_route(d)) {
                for arg in &def.parameters.args {
                    if let Some(id) = self.model.scope().get(arg.parameter.name.as_str()) {
        ...
        // 2. Propagate taint on assignments
        ...
        if let Expr::Call(call) = expr {
            ...
            if self.is_db_sink(call) {
                if call.arguments.args.iter().any(|arg| self.is_expr_tainted(arg)) {
                    let line = self.resolve_line(expr.into());
            ...
            // Sentinel Guard: crypto operations
            let crypto_ops = ["encrypt", "sign", "PBKDF2", "hash", "hmac"];
            let is_crypto = match call.func.as_ref() {
                Expr::Attribute(attr) => crypto_ops.contains(&attr.attr.as_str()),
                Expr::Name(name) => crypto_ops.contains(&name.id.as_str()),
                _ => false,
            };
            if is_crypto {
                let line = self.resolve_line(expr.into());
...
fn main() -> Result<()> {
    ...
    let parsed = parse_suite(source_file.source_text(), Mode::Module)?;

    // Build semantic model (basic; no cross-file resolution for MVP)
    let model_builder = SemanticModelBuilder::new(&source_file, &parsed, &Default::default());
    let semantic_model = model_builder.build(&parsed);
